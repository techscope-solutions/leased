  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <Link href="/" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
            <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
              ...
              <path d="M9 9h10M9 14h10M9 19h6" stroke="white" strokeWidth="1.8" strokeLinecap="round"/>
            </svg>
            <span style={{ fontStyle: 'italic', fontSize: 26, letterSpacing: '-0.02em', color: 'white', fontWeight: 500, fontFamily: 'Georgia, serif' }}>Leased</span>
          </Link>
          <p style={{ fontFamily: SF, fontSize: 14, color: 'rgba(255,255,255,0.4)', margin: 0 }}>
        ...
        <div style={{
          ...
          {emailSent ? (
            <div style={{ textAlign: 'center', padding: '16px 0' }}>
              ...
              <div style={{ fontFamily: SF, fontSize: 16, fontWeight: 600, color: 'white', marginBottom: 8 }}>Check your email</div>
              <p style={{ fontFamily: SF, fontSize: 13, color: 'rgba(255,255,255,0.45)', lineHeight: 1.6, margin: 0 }}>
                We sent a confirmation link to <strong style={{ color: 'rgba(255,255,255,0.7)' }}>{email}</strong>. Click it to activate your account.
              </p>
              <button
                onClick={() => { setEmailSent(false); setMode('signin'); }}
                style={{ marginTop: 20, background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', fontFamily: SF, fontSize: 13, cursor: 'pointer', textDecoration: 'underline' }}
              >
                Back to sign in
              </button>
            </div>
            ...
            <>
              ...
              <form onSubmit={handleEmail} style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
                ...
                {error && (
                  <div style={{
                    ...
                    borderRadius: 10, fontFamily: SF, fontSize: 12, color: 'rgba(255,100,60,0.9)',
                  }}>
                    {error}
                  </div>
                )}
                <button
                  ...
                  style={{
                    ...
                    background: emailLoading ? 'rgba(255,255,255,0.1)' : 'white',
                    border: 'none', borderRadius: 12,
                    color: '#0a0a0a',
                    fontFamily: SF, fontWeight: 600, fontSize: 14,
                    cursor: emailLoading ? 'not-allowed' : 'pointer',
                    opacity: (!email || !password) ? 0.5 : 1,
                    transition: 'all 0.15s',
                  }}
                >
                  {emailLoading ? <Spinner dark /> : (mode === 'signin' ? 'Sign in' : 'Create account')}
                </button>
              </form>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                ...
                <span style={{ fontFamily: SF, fontSize: 11, color: 'rgba(255,255,255,0.25)' }}>or</span>
                <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.08)' }} />
              </div>
        ...
        <p style={{ textAlign: 'center', marginTop: 20, fontFamily: SF, fontSize: 11, color: 'rgba(255,255,255,0.2)', lineHeight: 1.7 }}>
...
function Spinner({ dark = false }: { dark?: boolean }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" style={{ animation: 'spin 0.8s linear infinite' }}>
      ...
      <circle cx="9" cy="9" r="7" stroke={dark ? 'rgba(0,0,0,0.15)' : 'rgba(255,255,255,0.15)'} strokeWidth="2"/>
      <path d="M9 2a7 7 0 0 1 7 7" stroke={dark ? 'rgba(0,0,0,0.7)' : 'rgba(255,255,255,0.7)'} strokeWidth="2" strokeLinecap="round"/>
    </svg>
  );
}
